...
async function runEdgeCaseSuite() {
  ...
  assert(
    res1.status === 400,
    'Reject invalid calendar date string (400 Bad Request)',
    res1,
  );
  ...
  assert(
    res2.status === 400,
    'Reject past date booking attempt (400 Bad Request)',
    res2,
  );
  ...
  assert(
    ...
    'Book valid future STREAM appointment (201 Created)',
    bookingRes,
  );
  ...
  if (apptId) {
    ...
    assert(
      ...
      'Cancel active appointment (200 OK)',
      cancelRes1,
    );
    ...
    assert(
      cancelRes2.status === 400 ||
        cancelRes2.status === 409 ||
        cancelRes2.status === 200,
      'Handle repeat cancellation gracefully',
      cancelRes2,
    );

    // Reschedule already cancelled appointment
    const rescheduleCancelledRes = await request(
      `/appointment/${apptId}/reschedule`,
      {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${patToken}` },
        body: JSON.stringify({
          date: '2026-08-17',
          slot: { startTime: '11:00', endTime: '11:15' },
        }),
      },
    );
    assert(
      rescheduleCancelledRes.status === 400,
      'Reject rescheduling already cancelled appointment (400 Bad Request)',
      rescheduleCancelledRes,
    );
  }
  ...
  if (availId) {
    const previewRes = await request(
      `/doctor/availability/${availId}/shrink-preview?startTime=10:00&endTime=12:00`,
      {
        headers: { Authorization: `Bearer ${docToken}` },
      },
    );
    assert(
      previewRes.status === 200 &&
        typeof previewRes.data?.affectedCount === 'number',
      'Execute shrink-preview dry-run query (200 OK)',
      previewRes,
    );
  }
