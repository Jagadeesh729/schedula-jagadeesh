...
async function setup() {
  ...
  const d1Reg = await req('POST', '/auth/signup', {
    name: 'Dr Test One',
    email: d1Email,
    password: 'Pass1234!',
    role: 'DOCTOR',
  ...
  const d1Login = await req('POST', '/auth/login', {
    email: d1Email,
    password: 'Pass1234!',
  });
  ...
  const d2Reg = await req('POST', '/auth/signup', {
    name: 'Dr Test Two',
    email: d2Email,
    password: 'Pass1234!',
    role: 'DOCTOR',
  ...
  const d2Login = await req('POST', '/auth/login', {
    email: d2Email,
    password: 'Pass1234!',
  });
  ...
  const pReg = await req('POST', '/auth/signup', {
    name: 'Patient One',
    email: pEmail,
    password: 'Pass1234!',
    role: 'PATIENT',
...
async function runTests() {
  ...
  if (r.data?.id) {
    createdSlotId = r.data.id;
    console.log(`       Slot ID: ${createdSlotId}`);
  }
  ...
  check(
    'T04 — Create second non-overlapping slot → 201',
    r.status === 201,
    r.status,
  );
  ...
  check(
    'T05 — Adjacent slot (11:00 after 09:00–11:00) → 201 (allowed)',
    r.status === 201,
    r.status,
  );
  ...
  r = await req(
    'POST',
    '/doctor/availability',
    { weekday: 'Monday', startTime: '10:00', endTime: '13:00' },
    DOCTOR_TOKEN,
  );
  ...
  r = await req(
    'POST',
    '/doctor/availability',
    { weekday: 'Tuesday', startTime: '9:00', endTime: '10:00' },
    DOCTOR_TOKEN,
  );
  check(
    'T10 — Invalid time format (9:00 not HH:MM) → 400',
    r.status === 400,
    r.status,
  );
  ...
  if (createdSlotId) {
    r = await req(
      'PATCH',
      `/doctor/availability/${createdSlotId}`,
      { startTime: '08:00', endTime: '10:00' },
      DOCTOR_TOKEN,
    );
    check('T13 — Update own slot → 200', r.status === 200, r.status);
    check(
      'T14 — Updated startTime reflects change',
      r.data?.startTime?.startsWith('08'),
      r.data?.startTime,
    );
  ...
  check(
    'T18 — Invalid calendar date (2026-02-30) → 400',
    r.status === 400,
    r.status,
  );
  ...
  r = await req(
    'POST',
    '/doctor/availability/override',
    { date: '2026-13-01', startTime: '09:00', endTime: '10:00' },
    DOCTOR_TOKEN,
  );
  check(
    'T19 — Invalid calendar date (month 13) → 400',
    r.status === 400,
    r.status,
  );
  ...
  r = await req(
    'GET',
    '/doctor/availability/date?date=2026-08-04',
    null,
    DOCTOR_TOKEN,
  );
  check('T20 — GET by date with override → 200', r.status === 200, r.status);
  check(
    'T21 — Returns source: custom',
    r.data?.source === 'custom',
    r.data?.source,
  );
  check(
    'T22 — Does not return recurring',
    r.data?.source !== 'recurring',
    r.data?.source,
  );
  ...
  r = await req(
    'GET',
    '/doctor/availability/date?date=2026-08-10',
    null,
    DOCTOR_TOKEN,
  );
  check(
    'T23 — GET by date (Monday) with no override → 200',
    r.status === 200,
    r.status,
  );
  check(
    'T24 — Returns source: recurring',
    r.data?.source === 'recurring',
    r.data?.source,
  );
  ...
  check(
    'T25 — GET /date with no query param → 400',
    r.status === 400,
    r.status,
  );
}
...
async function regression() {
  ...
  check(
    'R01 — GET /doctor/profile still works → 200',
    r.status === 200,
    r.status,
  );
  ...
  check(
    'R02 — PATCH /doctor/profile still works → 200',
    r.status === 200,
    r.status,
  );
  ...
  check(
    'R03 — Login with wrong creds still returns 401',
    r.status === 401,
    r.status,
  );
}
