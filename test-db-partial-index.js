...
async function testDatabasePartialIndexInvariant() {
  ...
  try {
    ...
    `);
    console.log(
      '  ✓ Verified idx_stream_slot_unique Partial Unique Index in PostgreSQL.\n',
    );
    ...
    await client.query('DELETE FROM doctor_profiles WHERE id = $1', [
      mockDoctorId,
    ]);
    await client.query('DELETE FROM patient_profiles WHERE id IN ($1, $2)', [
      mockPatient1Id,
      mockPatient2Id,
    ]);
    ...
    console.log(
      `  ✓ Insert #1 Succeeded! Created Appointment ID: ${appointment1Id}\n`,
    );

    // 2. Direct raw SQL Insert #2 (Duplicate CONFIRMED status - Expected DB Violation)
    console.log(
      '[2/3] Executing raw SQL INSERT #2 with duplicate CONFIRMED status...',
    );
    ...
    if (!dbViolationOccurred) {
      console.error(
        '  ✗ FAIL: PostgreSQL allowed duplicate CONFIRMED slot insert!',
      );
      await client.query('DELETE FROM appointments WHERE date = $1', [
        mockDate,
      ]);
      await client.query('DELETE FROM doctor_profiles WHERE id = $1', [
        mockDoctorId,
    ...
    console.log(
      `  ✓ Insert #3 Succeeded after status transition to CANCELLED! Created ID: ${res3.rows[0].id}`,
    );
    ...
    console.log(
      '  ✓ PostgreSQL engine physically rejected duplicate CONFIRMED slot (23505).',
    );
    console.log(
      "  ✓ Partial Unique Index WHERE status = 'CONFIRMED' is operating correctly!\n",
    );
    process.exit(0);
  } catch (err) {
