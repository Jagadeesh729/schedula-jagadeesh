...
async function runConcurrencyStressTest() {
  ...
  if (docSignup.status !== 201) {
    console.error(
      `Failed doctor signup (${docSignup.status}):`,
      docSignup.data,
    );
    process.exit(1);
  }
  ...
  if (docProfileRes.status !== 201) {
    console.error(
      `Failed doctor profile creation (${docProfileRes.status}):`,
      docProfileRes.data,
    );
    process.exit(1);
  }
  ...
  console.log(
    '[3/5] Configuring STREAM scheduling strategy (slot: 15m, buffer: 5m)...',
  );
  ...
  if (stratRes.status !== 201 && stratRes.status !== 200) {
    console.error(
      `Failed to configure STREAM strategy (${stratRes.status}):`,
      stratRes.data,
    );
    process.exit(1);
  }
  ...
  console.log(
    `  ✓ Target Slot: ${targetSlotStartTime} - ${targetSlotEndTime}\n`,
  );
  ...
  console.log(
    `[5/5] FIRING 50 SIMULTANEOUS PARALLEL BOOKING REQUESTS AT ${targetSlotStartTime}...`,
  );
  ...
  console.log(
    `Successful Bookings (HTTP 201):  ${successCount}  (Expected: EXACTLY 1)`,
  );
  console.log(
    `Conflict Rejections (HTTP 409/400): ${conflictCount}  (Expected: EXACTLY 49)`,
  );
  console.log(
    `Unexpected Failures (HTTP 5xx):  ${errorCount}  (Expected: EXACTLY 0)`,
  );
  console.log(
    '-------------------------------------------------------------------',
  );
  ...
  const isInvariantPassed =
    successCount === 1 &&
    conflictCount === CONCURRENT_COUNT - 1 &&
    errorCount === 0;

  if (isInvariantPassed) {
    console.log(
      '  ✓ INVARIANT PASSED: Exactly 1 booking confirmed, 49 rejected with 409/400 Conflict.',
    );
    console.log(
      '  ✓ PostgreSQL Row Locks & Partial Unique Index physically prevented double-booking!\n',
    );
    process.exit(0);
  } else {
    console.error(
      '  ✗ INVARIANT FAILED: Double-booking or unexpected failure detected!',
    );
    process.exit(1);
  }
}
