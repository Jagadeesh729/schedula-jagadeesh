...
async function runAppointmentManagementTests() {
  ...
  const pat1Token = await registerUser(pat1Email, password, 'PATIENT', 'Patient One');
  ...
  const pat2Token = await registerUser(pat2Email, password, 'PATIENT', 'Patient Two');
  await request('/patient/profile', {
    method: 'POST',
  ...
  assert(emptyPatRes.status === 200 && Array.isArray(emptyPatRes.data) && emptyPatRes.data.length === 0, 'GET /appointment/my returns 200 OK [] when empty', emptyPatRes);

  // Test 2: Empty Doctor Appointments
  const emptyDocRes = await request('/doctor/appointments', {
    headers: { Authorization: `Bearer ${docToken}` },
  });
  assert(emptyDocRes.status === 200 && Array.isArray(emptyDocRes.data) && emptyDocRes.data.length === 0, 'GET /doctor/appointments returns 200 OK [] when empty', emptyDocRes);

  // Test 3: Reject booking for past date
  const pastDateRes = await request('/appointment', {
    method: 'POST',
    headers: { Authorization: `Bearer ${pat1Token}` },
    body: JSON.stringify({ doctorId, date: '2020-01-01', startTime: '10:00', endTime: '10:15' }),
  });
  assert(pastDateRes.status === 400, 'POST /appointment rejects past date (400)', pastDateRes);

  // Test 4: Reject booking for invalid/non-generated STREAM slot
  const invalidSlotRes = await request('/appointment', {
    method: 'POST',
    headers: { Authorization: `Bearer ${pat1Token}` },
    body: JSON.stringify({ doctorId, date: '2028-10-09', startTime: '10:10', endTime: '10:25' }),
  });
  assert(invalidSlotRes.status === 400, 'POST /appointment rejects invalid/non-generated STREAM slot (400)', invalidSlotRes);

  // Test 5: Successful STREAM Booking via POST /appointment
  const book1Res = await request('/appointment', {
    method: 'POST',
    headers: { Authorization: `Bearer ${pat1Token}` },
    body: JSON.stringify({ doctorId, date: '2028-10-09', startTime: '10:00', endTime: '10:15' }),
  });
  assert(book1Res.status === 201 && book1Res.data?.status === 'CONFIRMED', 'POST /appointment creates STREAM booking with CONFIRMED status (201)', book1Res);
  ...
  assert(dupBookRes.status === 409, 'POST /appointment prevents duplicate STREAM slot booking (409 Conflict)', dupBookRes);

  // Test 7: Non-patient booking attempt
  const docBookRes = await request('/appointment', {
    method: 'POST',
    headers: { Authorization: `Bearer ${docToken}` },
    body: JSON.stringify({ doctorId, date: '2028-10-09', startTime: '10:20', endTime: '10:35' }),
  });
  assert(docBookRes.status === 403, 'POST /appointment blocks non-patient booking attempt (403)', docBookRes);
  ...
  assert(pat1List.status === 200 && pat1List.data.length === 1 && pat1List.data[0].doctor?.name === 'Dr. Mgmt', 'GET /appointment/my returns patient appointments with doctor profile', pat1List);

  // Test 9: Patient 2 GET /appointment/my (Isolation)
  const pat2List = await request('/appointment/my', {
    headers: { Authorization: `Bearer ${pat2Token}` },
  });
  assert(pat2List.status === 200 && pat2List.data.length === 0, 'GET /appointment/my enforces patient isolation (Patient 2 sees 0 appointments)', pat2List);

  // Test 10: Doctor GET /doctor/appointments
  const docList = await request('/doctor/appointments', {
    headers: { Authorization: `Bearer ${docToken}` },
  });
  assert(docList.status === 200 && docList.data.length === 1 && docList.data[0].patient?.name === 'Patient One', 'GET /doctor/appointments returns doctor assigned appointments with patient profile', docList);
  ...
  assert(idorCancelRes.status === 403, 'PATCH /appointment/:id/cancel blocks non-owner cancellation (403 Forbidden IDOR)', idorCancelRes);
  ...
  assert(cancelRes.status === 200 && cancelRes.data?.status === 'CANCELLED', 'PATCH /appointment/:id/cancel updates status to CANCELLED (200 OK)', cancelRes);
  ...
  assert(reCancelRes.status === 400, 'PATCH /appointment/:id/cancel rejects already-cancelled appointment (400)', reCancelRes);
  ...
  assert(reBookRes.status === 201 && reBookRes.data?.status === 'CONFIRMED', 'Cancelled STREAM slot becomes available again for new booking (201 Created)', reBookRes);
  ...
  const wave1Res = await request('/appointment', {
    method: 'POST',
    headers: { Authorization: `Bearer ${pat1Token}` },
    body: JSON.stringify({ doctorId, date: '2028-10-16', scheduleType: 'WAVE', window: '10:00-11:00' }),
  });
  assert(wave1Res.status === 201 && wave1Res.data?.token === 1, 'WAVE Booking 1 assigns Token 1 (201)', wave1Res);
  ...
  assert(wave2Res.status === 201 && wave2Res.data?.token === 2, 'WAVE Booking 2 assigns Token 2 (201)', wave2Res);
  ...
  assert(waveRebookRes.status === 201 && waveRebookRes.data?.token === 1, 'WAVE Re-booking after cancellation reopens capacity & assigns collision-free Token 1 (201)', waveRebookRes);
  ...
  const [race1, race2] = await Promise.all([
    request('/appointment', {
      method: 'POST',
      headers: { Authorization: `Bearer ${pat1Token}` },
      body: JSON.stringify({ doctorId, date: '2028-10-23', startTime: '10:20', endTime: '10:35' }),
    }),
    request('/appointment', {
      method: 'POST',
      headers: { Authorization: `Bearer ${pat2Token}` },
  ...
  const raceSuccessCount = [race1, race2].filter((r) => r.status === 201).length;
  const raceConflictCount = [race1, race2].filter((r) => r.status === 409).length;
  assert(raceSuccessCount === 1 && raceConflictCount === 1, 'Concurrent STREAM booking race yields exactly 1 success (201) and 1 conflict (409)');
  ...
  const [waveRace1, waveRace2] = await Promise.all([
    request('/appointment', {
      method: 'POST',
      headers: { Authorization: `Bearer ${pat1Token}` },
      body: JSON.stringify({ doctorId, date: '2028-10-30', scheduleType: 'WAVE', window: '10:00-11:00' }),
    }),
    request('/appointment', {
      method: 'POST',
      headers: { Authorization: `Bearer ${pat2Token}` },
  ...
  const waveRaceSuccess = [waveRace1, waveRace2].filter((r) => r.status === 201).length;
  const waveRaceConflict = [waveRace1, waveRace2].filter((r) => r.status === 409).length;
  assert(waveRaceSuccess === 1 && waveRaceConflict === 1, 'Concurrent WAVE capacity boundary enforces maxCapacity=1 with 1 success and 1 conflict');
