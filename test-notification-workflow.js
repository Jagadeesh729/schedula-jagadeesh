...
async function runNotificationWorkflowSuite() {
  ...
  await request('POST', `/doctors/${doctorId}/scheduling`, {
  ...
  const bookRes = await request('POST', '/appointment/book', {
    doctorId,
    date: '2026-08-17',
    scheduleType: 'STREAM',
  ...
  assert(latestNotif1.isRead === false, 'Notification isRead defaults to false');

  // 3. Test APPOINTMENT_CANCELLED Notification
  console.log('\n[3/6] Testing APPOINTMENT_CANCELLED notification workflow...');
  const cancelRes = await request('PATCH', `/appointment/${appt1Id}/cancel`, null, patToken);
  assert(cancelRes.status === 200, 'Appointment 1 cancelled successfully (200)');
  ...
  const bookRes2 = await request('POST', '/appointment/book', {
    doctorId,
    date: '2026-08-17',
    scheduleType: 'STREAM',
  ...
  const reschedRes = await request('PATCH', `/appointment/${appt2Id}/reschedule`, {
    date: '2026-08-17',
    slot: { startTime: '10:20', endTime: '10:35' },
  }, patToken);
  assert(reschedRes.status === 200, 'Appointment 2 rescheduled successfully (200)');
  ...
  const readRes = await request('PATCH', `/notifications/${notifToMark}/read`, null, patToken);
  assert(readRes.status === 200, 'PATCH /notifications/:id/read returns 200 OK');
  ...
  const badUuidRes = await request('PATCH', '/notifications/invalid-uuid/read', null, patToken);
  assert(badUuidRes.status === 400, 'Invalid UUID notification ID returns 400 Bad Request');
